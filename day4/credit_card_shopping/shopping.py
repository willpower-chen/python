...
* %s *
* %s *
**********************************''' %(mylib.myljust('欢迎来到willpower商城', 30), mylib.myljust('%s 您好' %customer['name'], 30))
        print(welcome_info)
    ...
    def register():
        ...
        if not customer:
            ...
            while flag:
                ...
                username = mylib.validate_input(r'^\w{1,15}$','用户名:', hint='输入提示: 用户名必须是数字和字母的组合，长度不超过15个字符，输入r返回上级菜单')
                print(username)
                if username == 'r':
                    flag = False
                    continue
                ...
                while True:
                    password = mylib.validate_input(r'^\w{1,15}$', '密码: ', hint='输入提示: 密码长度介于6~15个字符，输入r返回上级菜单', is_pass=True)
                    if password == 'r':
                        flag = False
                        break
